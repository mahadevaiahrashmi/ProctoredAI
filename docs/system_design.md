...
| Language | TypeScript 5 | Strict mode on (but build ignores errors — see §7) |
...
| `src/app/submitted/page.tsx` | **Deprecated** placeholder (not in active flow). |
...
| Build | `npm run build` | **Ignores TS/ESLint errors** |
...
- **Build safety:** `ignoreBuildErrors`/`ignoreDuringBuilds` can ship type/lint regressions; mitigate with separate CI gates (see [testing.md](testing.md)).
